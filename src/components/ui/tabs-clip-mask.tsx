...
export function TabsClipPath() {
  ...
  return (
    <div className="relative flex flex-col items-center w-fit mx-auto pt-16">
        ...
        {TABS.map((tab) => (
          <li key={tab.name}>
            <button
              ...
              className="flex h-[34px] items-center gap-2 rounded-full px-4 text-sm font-medium text-black"
            >
              {tab.icon}
              {tab.name}
            </button>
      ...
      <div 
        aria-hidden 
        className="absolute z-10 w-full overflow-hidden transition-[clip-path] duration-300 ease-out" 
        ref={containerRef}
          ...
          {TABS.map((tab) => (
            <li key={tab.name}>
              <button
                ...
                className="flex h-[34px] items-center gap-2 rounded-full px-4 text-sm font-medium text-white"
                tabIndex={-1}
              >
                {tab.icon}
                {tab.name}
