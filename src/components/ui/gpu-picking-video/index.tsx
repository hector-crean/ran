...
const VideoMesh = React.memo(
  React.forwardRef<VideoMeshHandle, VideoMeshProps>(function VideoMesh(
    {
      mainVideoElement,
      maskVideoElement,
      onMaskHover,
    ...
    const [maskTexture, setMaskTexture] = useState<THREE.VideoTexture | null>(
      null
    );
    ...
    useEffect(() => {
      if (mainVideoElement) {
        console.log("Main video properties:", {
          ...
          aspectRatio:
            mainVideoElement.videoWidth / mainVideoElement.videoHeight,
    ...
    useEffect(() => {
      if (maskVideoElement) {
        console.log("Mask video properties:", {
          ...
          aspectRatio:
            maskVideoElement.videoWidth / maskVideoElement.videoHeight,
        ...
        if (mainVideoElement && maskVideoElement) {
          ...
          if (aspectDiff > 0.01) {
            console.warn(
              "⚠️ Video aspect ratios don't match! This may cause alignment issues."
            );
          }
        }
        ...
        try {
          ...
          console.log(
            "✅ Mask video texture created successfully with NearestFilter for exact colors"
          );
        } catch (error) {
    ...
    const pickMask = useCallback(
      (x: number, y: number) => {
        ...
        gl.readRenderTargetPixels(pickingFBO, x, flippedY, 1, 1, pixelBuffer);
    ...
    useEffect(() => {
      if (materialRef.current) {
        materialRef.current.uniforms.uOutlineThickness.value =
          outlineSettings.thickness;
        materialRef.current.uniforms.uEdgeThreshold.value =
          outlineSettings.edgeThreshold;
        materialRef.current.uniforms.uLineInnerColor.value = new THREE.Color(
          outlineSettings.innerColor
        );
        materialRef.current.uniforms.uLineOuterColor.value = new THREE.Color(
          outlineSettings.outerColor
        );
        materialRef.current.uniforms.uMaskOverlayColor.value = new THREE.Color(
          outlineSettings.maskOverlayColor
        );
        materialRef.current.uniforms.uMaskOverlayOpacity.value =
          outlineSettings.maskOverlayOpacity;
        ...
        if (hoveredObject !== null) {
          ...
          materialRef.current.uniforms.uHoveredObject.value = new THREE.Vector3(
            r,
            g,
            b
          );
          console.log("Setting hovered object color:", {
            ...
            exact_rgb: [
              (hoveredObject >> 16) & 0xff,
              (hoveredObject >> 8) & 0xff,
              hoveredObject & 0xff,
            ],
        ...
        if (selectedObject !== null) {
          ...
          materialRef.current.uniforms.uSelectedObject.value =
            new THREE.Vector3(r, g, b);
          console.log("Setting selected object color:", {
            objectId: selectedObject,
            exact_rgb: [
              (selectedObject >> 16) & 0xff,
              (selectedObject >> 8) & 0xff,
              selectedObject & 0xff,
            ],
            normalized_rgb: [r, g, b],
  ...
  useFrame(state => {
    if (camera && "zoom" in camera) {
      ...
      const breathe =
        ...
        breatheAmount *
          0.5 *
          (1 + Math.sin(state.clock.elapsedTime * breatheSpeed));
...
export const GpuPickingVideo = React.forwardRef<
  GpuPickingVideoHandle,
  GpuPickingVideoProps
>(function GpuPickingVideo(
  ...
  const [maskVideoElement, setMaskVideoElement] =
    useState<HTMLVideoElement | null>(null);
  ...
  return (
    <div className={`relative isolate h-full w-full ${className} `}>
      ...
      {/* Status Display */}
      <div className="absolute top-4 right-4 rounded bg-black/70 px-3 py-2 text-sm text-white">
        {error ? (
      ...
      {maskInfo && (
        <div className="absolute top-16 right-4 rounded bg-black/70 px-3 py-2 text-xs text-white">
          <div>
            Cursor: ({maskInfo.x}, {maskInfo.y})
          </div>
          <div>Object ID: {maskInfo.colorInfo.objectId}</div>
          <div>
            RGB: ({maskInfo.colorInfo.r}, {maskInfo.colorInfo.g},{" "}
            {maskInfo.colorInfo.b})
          </div>
          ...
          <div className="mt-2 space-y-1">
            <div
              className={`rounded px-2 py-1 text-xs ${
                hoveredObject === maskInfo.colorInfo.objectId
            ...
            <div
              className={`rounded px-2 py-1 text-xs ${
                selectedObject === maskInfo.colorInfo.objectId
      ...
      {/* Debug Info */}
      <div className="absolute top-4 left-4 rounded bg-black/70 px-3 py-2 text-xs text-white">
        ...
        <div>
          GPU Picking:{" "}
          {isVideoLoaded && isMaskVideoLoaded ? "Active" : "Inactive"}
        </div>
        ...
        {mainVideoElement && (
          <div className="mt-2 border-t border-gray-600 pt-2">
            ...
            <div>
              Resolution: {mainVideoElement.videoWidth}x
        ...
        {maskVideoElement && (
          <div className="mt-2 border-t border-gray-600 pt-2">
            ...
            <div>
              Resolution: {maskVideoElement.videoWidth}x
              {maskVideoElement.videoHeight}
            </div>
            <div>
              Aspect:{" "}
              {(
        ...
        <div className="mt-2 border-t border-gray-600 pt-2">
          <div>
            Hovered:{" "}
            {hoveredObject !== null
              ? `#${hoveredObject.toString(16).padStart(6, "0")}`
              : "None"}
          </div>
          <div>
            Selected:{" "}
            {selectedObject !== null
              ? `#${selectedObject.toString(16).padStart(6, "0")}`
              : "None"}
          </div>
        </div>
      </div>
