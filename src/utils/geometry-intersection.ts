...
function handlePointIntersection(
  point: Extract<Geometry, { type: "Point" }>,
  geom2: Geometry
): boolean {
  switch (geom2.type) {
    case "Point":
      return (
        point.coordinates[0] === geom2.coordinates[0] &&
...
function handlePolygonIntersection(
  poly: Extract<Geometry, { type: "Polygon" }>,
  geom2: Geometry
): boolean {
  switch (geom2.type) {
    case "Point":
      return pointInPolygon(
        geom2.coordinates as [number, number],
        poly.coordinates[0]
      );
    case "Polygon":
...
function handleMultiPolygonIntersection(
  multiPoly: Extract<Geometry, { type: "MultiPolygon" }>,
  geom2: Geometry
): boolean {
...
function handleLineStringIntersection(
  line: Extract<Geometry, { type: "LineString" }>,
  geom2: Geometry
): boolean {
...
function handleMultiLineStringIntersection(
  multiLine: Extract<Geometry, { type: "MultiLineString" }>,
  geom2: Geometry
): boolean {
...
function handleMultiPointIntersection(
  multiPoint: Extract<Geometry, { type: "MultiPoint" }>,
  geom2: Geometry
): boolean {
